...
*Last updated: 2026-02-17 21:55 UTC. Tests: 135 Rust (109 HTTP + 14 integration + 12 unit) + 188 Python SDK integration. All passing.*

- ✅ **Python SDK** — Complete zero-dependency client library (`sdk/python/kanban.py`). Covers all API endpoints: boards, columns, tasks, batch operations, comments, activity feed, search, dependencies, webhooks, SSE streaming, discovery. Convenience helpers (find_column by name, move_task_to, quick_task). Typed error hierarchy. 188 integration tests.
...
### Completed (2026-02-18 Daytime, Session — 16:15 UTC)

- **Rust HTTP test expansion 147→187 (+40)** ✅ Done — 40 new HTTP integration tests covering: activity feed enrichment (created events with task snapshots, comment events with recent_comments), activity cursor pagination (after_seq ASC, since filter, limit), task events endpoint (all event types, new task events), WIP limit enforcement on move (blocks when at capacity), completed_at lifecycle (set on move to Done, cleared on move away), multi-criteria filters (priority+label, assigned+column), reorder with cross-column move, dependency CRUD and self-dependency rejection, comment count accuracy, board isolation (cross-key rejection, task count), metadata persistence+update, column reorder preserving tasks, board settings all fields, webhook deactivate/reactivate, task column change via update, search pagination, activity limit, WIP limit update, multiple boards same name, archived board blocking writes, due date lifecycle, batch update/delete, move event column names, board descriptions, list/archive filtering, stale filter, label normalization, board timestamps, webhook multiple events, search special chars safety, assigned_to change, public board creation. Plus helper function `get_first_column_id`. Zero clippy warnings. All 213 Rust tests pass (187 HTTP + 14 integration + 12 unit). Commit: (pending).
